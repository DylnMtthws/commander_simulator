...
constexpr std::uint32_t rotate(std::uint32_t value, int amount) {
...
std::string sha256_hex(std::string_view input) {
    ...
    for (std::size_t offset = 0; offset < bytes.size(); offset += 64U) {
        ...
        for (std::size_t i = 16; i < words.size(); ++i) {
            const std::uint32_t s0 = rotate(words[i - 15U], 7) ^ rotate(words[i - 15U], 18) ^
                                     (words[i - 15U] >> 3U);
            const std::uint32_t s1 = rotate(words[i - 2U], 17) ^ rotate(words[i - 2U], 19) ^
                                     (words[i - 2U] >> 10U);
        ...
        for (std::size_t i = 0; i < words.size(); ++i) {
            ...
            const std::uint32_t temp1 = h + s1 + choose + kRound[i] + words[i];
